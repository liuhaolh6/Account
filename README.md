...
二者服务于不同目的——前者用于判断是否超支，后者用于核对筛选结果。
因此**没有**把两者强行统一，而是在界面上分别标注了统计范围，避免用户误以为算错。
历史上预算页曾因此被误判为 bug：早期 `spentCents` 只在 ViewModel 初始化时查询一次，
新增流水后不刷新，现已改为订阅 `observeSummarySource` 的 Flow 持续重算。
